...
const CartItem = ({ item }) => {
  const { removeFromCart, updateQuantity } = useCartStore(); // Extract methods from store

  const handleDecreaseQuantity = () => {
    if (item.quantity > 1) {
      console.log("Decreasing quantity for:", item.name); // Log action
      updateQuantity(item._id, item.quantity - 1); // Decrease quantity
    }
  };

  const handleIncreaseQuantity = () => {
    console.log("Increasing quantity for:", item.name); // Log action
    updateQuantity(item._id, item.quantity + 1); // Increase quantity
  };
        ...
        <div className="flex items-center justify-between md:order-3 md:justify-end">
          <div className="flex items-center gap-2">
            <button
              ...
              onClick={handleDecreaseQuantity} // Decrease quantity handler
            >
            ...
            <button
              className="inline-flex h-5 w-5 shrink-0 items-center justify-center rounded-md border border-gray-600 bg-gray-700 hover:bg-gray-600 focus:outline-none focus:ring-2 focus:ring-emerald-500"
              onClick={handleIncreaseQuantity} // Increase quantity handler
            >
          ...
          <div className="text-end md:order-4 md:w-32">
            {/* คำนวณราคาตามจำนวนสินค้า */}
            <p className="text-base font-bold text-emerald-400">
              ${item.price * item.quantity} {/* แสดงผลราคาคำนวณใหม่ */}
            </p>
          </div>
        </div>
